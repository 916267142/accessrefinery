...
- **Frameworks:** We design the Multi-round Constraints Preprocessor (**MCP**) as a data structure that accelerates multi-round SMT solving. We also package **MCP** as a Java JAR to facilitate reuse.
...
- **Documented:** The artifact provides documentation for:
  - Requirements
  - Installation
  - Project structure
  - Usage and examples
...
- **Complete:** The artifact includes all materials required to reproduce the reported experiments:
  ...
  - Reproduction scripts and instructions
  - Plotting scripts
- **Consistency and Evidence:** The released artifact is consistent with the evaluation conclusions reported in the paper. We provide archived results and instructions to map those results to the paper's conclusions.

## Evaluated - Reusable

We believe this artifact satisfies the Reusable criteria (*The artifacts are carefully documented and well-structured to facilitate reuse and repurposing*) for the following reasons:
...
- **Reuse and Repurposing of MCP** (Library-Level Reuse): We provide the following evidence to support direct reuse and extension of **MCP** in other projects:
  ...
  - **Standardized API Documentation:** We provide standardized comments in the **AccessRefinery** codebase, enabling automatic Maven documentation generation.
